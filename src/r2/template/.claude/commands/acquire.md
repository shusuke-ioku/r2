...
2. Download via Sci-Hub: `PYTHONPATH=.claude .venv/bin/python -m rag lit-download "IDENTIFIER" --type TYPE --title "TITLE"`
3. Verify Zotero addition (mandatory -- every paper must be in Zotero)
4. Verify RAG indexing (check chunk count in output)
5. For batch: `PYTHONPATH=.claude .venv/bin/python -m rag lit-download-batch 'JSON' --auto-index`
